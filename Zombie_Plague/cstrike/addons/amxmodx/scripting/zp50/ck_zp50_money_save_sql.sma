...
public plugin_cfg()
{
	...
	{
		new Handle:SQL_Query_Check_DB = SQL_PrepareQuery(g_SQL_Connect, "CREATE TABLE IF NOT EXISTS `zp_money` (Player_ID INT PRIMARY KEY auto_increment, Steam VARCHAR(32), Money INT");
...
public client_putinserver(iPlayer)
{
	...
	formatex(szQuery, charsmax(szQuery), "SELECT * FROM `zp_money` WHERE `Steam` LIKE '%s'", szAuth_ID);
...
public SQL_Check_Player(iFail_State, Handle:SQL_Query, szError[], iError, szData[])
{
	...
	else
	{
		...
		(
			g_SQL_Connect,
			"INSERT INTO `zp_money` VALUES (NULL, '%s', 0)",
			szAuth_ID
		);
...
public client_disconnected(iPlayer)
{
	...
	(
		g_SQL_Connect,
		"UPDATE `zp_money` SET `Money` = %d WHERE `Player_ID` = %d",
...
{
	new Handle:SQL_Query_Get_Last_Index = SQL_PrepareQuery(g_SQL_Connect, "SELECT `Player_ID` FROM `zp_money` ORDER BY `Player_ID` DESC;");
