...
public plugin_init()
{
	...
	g_pCvar_Assassin_Health = register_cvar("zm_assassin_health", "0");
	g_pCvar_Assassin_Base_Health = register_cvar("zm_assassin_base_health", "2000");
	g_pCvar_Assassin_Speed = register_cvar("zm_assassin_speed", "1.05");
	g_pCvar_Assassin_Gravity = register_cvar("zm_assassin_gravity", "0.5");
	g_pCvar_Assassin_Glow = register_cvar("zm_assassin_glow", "1");

	g_pCvar_Assassin_Aura = register_cvar("zm_assassin_aura", "1");
	g_pCvar_Assassin_Aura_Radius = register_cvar("zm_assassin_aura_radius", "1");
	g_pCvar_Assassin_Aura_Color_R = register_cvar("zm_assassin_aura_color_R", "150");
	g_pCvar_Assassin_Aura_Color_G = register_cvar("zm_assassin_aura_color_G", "0");
	g_pCvar_Assassin_Aura_Color_B = register_cvar("zm_assassin_aura_color_B", "0");
	g_pCvar_Assassin_Aura_Life = register_cvar("zm_assassin_aura_life", "2");
	g_pCvar_Assassin_Aura_Decay_Rate = register_cvar("zm_assassin_aura_decay_rate", "0");

	g_pCvar_Assassin_Kill_Explode = register_cvar("zm_assassin_kill_explode", "1");
	g_pCvar_Assassin_Damage = register_cvar("zm_assassin_damage", "1000");

	g_pCvar_Assassin_Grenade_Frost = register_cvar("zm_assassin_grenade_frost", "0");
	g_pCvar_Assassin_Grenade_Fire = register_cvar("zm_assassin_grenade_fire", "1");

	g_Forward = CreateMultiForward("zp_fw_class_asassin_bit_change", ET_CONTINUE, FP_CELL);
...
public plugin_precache()
{
	...
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Player Models", "ASSASSIN", g_aModels_Assassin_Player);
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Weapon Models", "V_KNIFE ASSASSIN", g_aModels_Assassin_Claw);

	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN DIE", g_aSound_Assassin_Die);
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN FALL", g_aSound_Assassin_Fall);
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN PAIN", g_aSound_Assassin_Pain);
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN MISS SLASH", g_aSound_Assassin_Miss_Slash);
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN MISS WALL", g_aSound_Assassin_Miss_Wall);
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN HIT NORMAL", g_aSound_Assassin_Hit_Normal);
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN HIT STAB", g_aSound_Assassin_Hit_Stab);

	// If we couldn't load from file, use and save default ones
	if (ArraySize(g_aModels_Assassin_Player) == 0)
	{
		for (new i = 0; i < sizeof g_Models_Assassin_Player; i++)
		{
			ArrayPushString(g_aModels_Assassin_Player, g_Models_Assassin_Player[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Player Models", "ASSASSIN", g_aModels_Assassin_Player);
	}

	if (ArraySize(g_aModels_Assassin_Claw) == 0)
	{
		for (new i = 0; i < sizeof g_Models_Assassin_Claw; i++)
		{
			ArrayPushString(g_aModels_Assassin_Claw, g_Models_Assassin_Claw[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Weapon Models", "V_KNIFE ASSASSIN", g_aModels_Assassin_Claw);
	}

	if (ArraySize(g_aSound_Assassin_Die) == 0)
	{
		for (new i = 0; i < sizeof g_Sound_Assassin_Die; i++)
		{
			ArrayPushString(g_aSound_Assassin_Die, g_Sound_Assassin_Die[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN DIE", g_aSound_Assassin_Die);
	}

	if (ArraySize(g_aSound_Assassin_Fall) == 0)
	{
		for (new i = 0; i < sizeof g_Sound_Assassin_Fall; i++)
		{
			ArrayPushString(g_aSound_Assassin_Fall, g_Sound_Assassin_Fall[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN FALL", g_aSound_Assassin_Fall);
	}

	if (ArraySize(g_aSound_Assassin_Pain) == 0)
	{
		for (new i = 0; i < sizeof g_Sound_Assassin_Pain; i++)
		{
			ArrayPushString(g_aSound_Assassin_Pain, g_Sound_Assassin_Pain[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN PAIN", g_aSound_Assassin_Pain)
	}

	if (ArraySize(g_aSound_Assassin_Miss_Slash) == 0)
	{
		for (new i = 0; i < sizeof g_Sound_Assassin_Miss_Slash; i++)
		{
			ArrayPushString(g_aSound_Assassin_Miss_Slash, g_Sound_Assassin_Miss_Slash[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN MISS SLASH", g_aSound_Assassin_Miss_Slash)
	}

	if (ArraySize(g_aSound_Assassin_Miss_Wall) == 0)
	{
		for (new i = 0; i < sizeof g_Sound_Assassin_Miss_Wall; i++)
		{
			ArrayPushString(g_aSound_Assassin_Miss_Wall, g_Sound_Assassin_Miss_Wall[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN MISS WALL", g_aSound_Assassin_Miss_Wall);
	}

	if (ArraySize(g_aSound_Assassin_Hit_Normal) == 0)
	{
		for (new i = 0; i < sizeof g_Sound_Assassin_Hit_Normal; i++)
		{
			ArrayPushString(g_aSound_Assassin_Hit_Normal, g_Sound_Assassin_Hit_Normal[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN HIT NORMAL", g_aSound_Assassin_Hit_Normal);
	}

	if (ArraySize(g_aSound_Assassin_Hit_Stab) == 0)
	{
		for (new i = 0; i < sizeof g_Sound_Assassin_Hit_Stab; i++)
		{
			ArrayPushString(g_aSound_Assassin_Hit_Stab, g_Sound_Assassin_Hit_Stab[i]);
		}

		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Sounds", "ASSASSIN HIT STAB", g_aSound_Assassin_Hit_Stab);
	}
...
public RG_CBasePlayer_TakeDamage_(iVictim, iInflictor, iAttacker, Float:fDamage)
{
	...
	if (BIT_VALID(g_iBit_Assassin, iAttacker) && !zp_core_is_zombie(iVictim))
	{
		...
		{
			// Set assassin damage
			SetHookChainArg(4, ATYPE_FLOAT, fDamage * float(get_pcvar_num(g_pCvar_Assassin_Damage))); // ExecuteHamB(Ham_Killed, victim, attacker, 0)
		}
	}
...
public zp_fw_core_infect_post(iPlayer)
{
	...
	// Health
	if (get_pcvar_num(g_pCvar_Assassin_Health) == 0)
	{
		SET_USER_HEALTH(iPlayer, float(get_pcvar_num(g_pCvar_Assassin_Base_Health)) * Get_Alive_Count());
	}

	else
	{
		SET_USER_HEALTH(iPlayer, float(get_pcvar_num(g_pCvar_Assassin_Health)));
	}
	...
	cs_set_player_model(iPlayer, g_Models_Assassin_Player[random(sizeof g_Models_Assassin_Player)]);
	...
	if (get_pcvar_num(g_pCvar_Assassin_Glow))
	{
		rh_set_user_rendering(iPlayer, kRenderFxGlowShell, get_pcvar_num(g_pCvar_Assassin_Aura_Color_R), get_pcvar_num(g_pCvar_Assassin_Aura_Color_G), get_pcvar_num(g_pCvar_Assassin_Aura_Color_B), kRenderNormal, 25);
	}
