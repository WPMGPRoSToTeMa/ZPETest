...
new const g_Sound_Nemesis_Hit_Solid[][] =
{
...
public plugin_precache()
{
	...
	for (new i = 0; i < sizeof g_Sound_Nemesis_Hit_Solid; i++)
	{
		precache_sound(g_Sound_Nemesis_Hit_Solid[i]);
	}
	...
	if (BIT_VALID(g_iBit_Nemesis, iPlayer))
	{
		...
		if (szSample[8] == 'k' && szSample[9] == 'n' && szSample[10] == 'i')
		{
			...
			if (szSample[14] == 'h' && szSample[15] == 'i' && szSample[16] == 't')
			{
				if (szSample[18] == 's')
				{
					emit_sound(iPlayer, iChannel, g_Sound_Nemesis_Hit_Solid[RANDOM(sizeof g_Sound_Nemesis_Hit_Solid)], fVolume, fAttn, iFlags, iPitch);
